...
StickyCell.prototype.update = function(gameServer) {
    ...
    for (var i in gameServer.nodesPlayer) {
        ...
        var collisionDist = check.getSize() + this.radius;
        var trueDist = (check.position.x - this.position.x)
                      *(check.position.x - this.position.x)
                      +(check.position.y - this.position.y)
                      *(check.position.y - this.position.y);
        if(trueDist > collisionDist*collisionDist) {
...
// TODO: steal the following two functions from MotherCell if it becomes accessible
StickyCell.prototype.abs = function(n) {
    // Because Math.abs is slow
    return (n < 0) ? -n : n;
};
StickyCell.prototype.visibleCheck = function(box, centerPos) {
