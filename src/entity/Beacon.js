...
Beacon.prototype.feed = function(feeder, gameServer) {
    ...
    if(Math.random() < 0.25) {
        this.spawnEjected(gameServer, feeder.owner, gameServer.getRandomColor());
    }
    ...
    if(this.stage % 20 === 0) {
        ...
        moving.setMoveEngineData(15+10*Math.random(), Infinity, 1);
    ...
    if(this.stage >= this.maxStage) {
        ...
        for(var i = 0; i < this.maxStage/4; i++) {
            this.spawnEjected(gameServer, feeder.owner, color);
        }
...
// TODO: take the following 3 methods from mother cell once possible
Beacon.prototype.abs = function(n) {
    // Because Math.abs is slow
    return (n < 0) ? -n : n;
};
Beacon.prototype.visibleCheck = function(box, centerPos) {
...
Beacon.prototype.spawnFood = function(gameServer) {
...
Beacon.prototype.spawnEjected = function(gameServer, owner, parentColor) {
    // Get starting position
    var angle = Math.random() * 6.28; // (Math.PI * 2) ??? Precision is not our greatest concern here
    var r = this.getSize();
    var pos = {
    ...
    // Spawn food
    var f = new EjectedMass(gameServer.getNextNodeId(), owner, pos, gameServer.config.ejectMass, gameServer);
